    ...
    public partial class CadastroVenda : CadastroBase
    {
        ...
        private void btnAdicionar_Click(object sender, EventArgs e)
        {
            if (ValidaItem())
            {
                ...
                if (float.TryParse(txtVIUnitario.Text, NumberStyles.Currency, CultureInfo.CurrentCulture.NumberFormat, out var vlUnitario))
                {
        ...
        {
            if (double.TryParse(txtVIUnitario.Text, out double value))
                txtVIUnitario.Text = string.Format(CultureInfo.CurrentCulture, @"{0:C2}", value);
            else
                txtVIUnitario.Text = string.Empty;
        ...
        {
            var convVlr = float.TryParse(txtVIUnitario.Text, NumberStyles.Currency, CultureInfo.CurrentCulture.NumberFormat, out float vlUnitario);
            var convQtd = int.TryParse(txtQuantidade.Text, out int quantidade);
            if (convVlr && convQtd)
            {
                var valorTotal = quantidade * vlUnitario;
                txtVITotal.Text = string.Format(CultureInfo.CurrentCulture, "{0:C2}", valorTotal);
            }
        }

        private void CalculaTotalVenda()
        {
            IbIValor.Text = $@"Valor Total: {string.Format(CultureInfo.CurrentCulture, "{0:C2}", _vendaItems.Sum(x => x.ValorTotal))}";
            IbIQtdItens.Text = $@"Qtd. Produtos: {_vendaItems.Sum(x => x.Quantidade)}";
        }
